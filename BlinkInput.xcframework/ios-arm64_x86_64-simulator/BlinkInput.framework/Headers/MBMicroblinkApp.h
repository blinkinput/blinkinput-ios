...
static inline NSString * MBI_LOCALIZED_DEFAULT_STRING(NSString *key) {
    return ([NSString stringWithFormat:@"[[%@]]", key]);
}

static inline NSString * MBI_LOCALIZED_FOR_LANGUAGE(NSString *key, NSString *language) {

    NSString *overridenStringFromCustomBundle = NSLocalizedStringWithDefaultValue(key,
                                                                                  language,
                                                                                  [MBIMicroblinkApp sharedInstance].customResourcesBundle,
                                                                                  @"",
                                                                                  @"");
    ...
    return NSLocalizedStringWithDefaultValue(key,
                                             language,
                                             [MBIMicroblinkApp sharedInstance].resourcesBundle,
                                             MBI_LOCALIZED_DEFAULT_STRING(key),
                                             nil);
}

static inline NSString * MBI_LOCALIZED(NSString *key) {
    NSString *localizationFileName = [MBIMicroblinkApp sharedInstance].customLocalizationFileName;
    ...
    NSString *language = [MBIMicroblinkApp sharedInstance].language;
    NSString *localizedString = MBI_LOCALIZED_FOR_LANGUAGE(key, language);
    
#ifndef DNDEBUG
    return localizedString;
#else
    if ([localizedString isEqualToString:MBI_LOCALIZED_DEFAULT_STRING(key)]) {
        if (![language isEqualToString:@"ar-AE"] && [language hasPrefix:@"ar"]) {
            localizedString = MBI_LOCALIZED_FOR_LANGUAGE(key, @"ar-AE");
        }
        else if (![language isEqualToString:@"en"]) {
            localizedString = MBI_LOCALIZED_FOR_LANGUAGE(key, @"en");
        }
    }
    return localizedString;
#endif
