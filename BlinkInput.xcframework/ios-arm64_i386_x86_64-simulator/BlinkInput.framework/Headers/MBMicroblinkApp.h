...
@interface MBINMicroblinkApp : NSObject
...
static inline NSString * MBIN_LOCALIZED_DEFAULT_STRING(NSString *key) {
    return ([NSString stringWithFormat:@"[[%@]]", key]);
}

static inline NSString * MBIN_LOCALIZED_FOR_LANGUAGE(NSString *key, NSString *language) {

    NSString *overridenStringFromCustomBundle = NSLocalizedStringWithDefaultValue(key,
                                                                                  language,
                                                                                  [MBINMicroblinkApp sharedInstance].customResourcesBundle,
                                                                                  @"",
                                                                                  @"");
    ...
    return NSLocalizedStringWithDefaultValue(key,
                                             language,
                                             [MBINMicroblinkApp sharedInstance].resourcesBundle,
                                             MBIN_LOCALIZED_DEFAULT_STRING(key),
                                             nil);
}

static inline NSString * MBIN_LOCALIZED(NSString *key) {
    NSString *localizationFileName = [MBINMicroblinkApp sharedInstance].customLocalizationFileName;
    ...
    NSString *localizedString = MBIN_LOCALIZED_FOR_LANGUAGE(key, [MBINMicroblinkApp sharedInstance].language);
    
#ifndef DNDEBUG
    return localizedString;
#else
    if ([localizedString isEqualToString:MBIN_LOCALIZED_DEFAULT_STRING(key)] && ![MBINMicroblinkApp.sharedInstance.language isEqualToString:@"en"]) {
        localizedString = MBIN_LOCALIZED_FOR_LANGUAGE(key, @"en");
    }
    return localizedString;
#endif
}
