 ...
 */
NS_INLINE MBINRange MBINMakeRange(CGFloat start, CGFloat stop) {
    MBINRange r;
    r.start = start;
    r.stop = stop;
    return r;
 ...
 */
NS_INLINE MBINScale MBINMakeScale(CGFloat scale, CGFloat tolerance) {
    MBINScale r;
    r.scale = scale;
...
@interface MBINDocumentSpecification : NSObject <NSCopying>
 ...
 */
+ (instancetype)createFromPreset:(MBINDocumentSpecificationPreset)preset;

/**
 * Sets scale and scale tolerance that will be used when detecting document in both orientations.
 */
- (void)setPortraitAndLandscapeScale:(MBINScale)scale;
 ...
 *
 * Default: MBINMakeScale(1.0, 0.0)
 */
@property (nonatomic, assign) MBINScale portraitScale;
 ...
 * Default: MBINMakeScale(1.0, 0.0)
 */
@property (nonatomic, assign) MBINScale landscapeScale;

/**
 * Scanning mode that defines in which orientations can this document be detected.
 *
 * Default: MBINScanningModeAuto
 */
@property (nonatomic, assign) MBINScanningMode scanningMode;

/**
 * Percentage of possible document offset on x axis.
 *
 * Default: MBINMakeRange(-1, -1);
 */
@property (nonatomic, assign) MBINRange xRange;
 ...
 * Default: MBINMakeRange(-1, -1);
 */
@property (nonatomic, assign) MBINRange yRange;
