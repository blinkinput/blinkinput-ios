...
- (PPCameraCoordinator *)coordinatorWithError:(NSError **)error {
    ...
    if ([appID isEqualToString:@"com.microblink.USCheque-Sample"]) {
        ...
        settings.licenseSettings.licenseKey =
            @"4W4JKWIP-GUSUWVWN-ZDTWROI3-FFFVNDMD-BCIF77RQ-TKQ5GIDB-4YC52RKM-52O6ZZZD"; // Valid temporarily
    } else if ([appID isEqualToString:@"com.microblink.photopay.checkscan"]) {
        // Visit www.microblink.com to get the license key for your app
        settings.licenseSettings.licenseKey =
            @"5SKB73YC-ER2FPW3K-PL5Y3T47-MHFVZCJL-SN3ZNKXX-75OYDSAN-RPQKRPMS-C37BOC6R"; // Valid temporarily
    }
    ...
    /*------------------------------------*/
    {
        ...
        /** Setup reading of suspected OCR line with all awailable fonts */
        {
             ...
             */
            PPRegexOcrParserFactory *classificationParser =
                [[PPRegexOcrParserFactory alloc] initWithRegex:@"([+-<>]?\\d{2,}[+-<>]? ?){3,10}"];
             ...
             */
            [classificationInfosArray addObject:[[PPDecodingInfo alloc] initWithLocation:ocrLineLocation
                                                                          dewarpedHeight:dewarpHeight
                                                                                uniqueId:US_CHEQUE_CLASSIFICATION]];
        }
        ...
        /** Setup reading of confirmed OCR line with MICR font only */
        {
            ...
            PPRegexOcrParserFactory *ocrLineParser = [[PPRegexOcrParserFactory alloc]
                initWithRegex:[NSString stringWithFormat:@"(%@)|(%@)|(%@)", chequeRegexOne, chequeRegexTwo, chequeRegexThree]];
             ...
             * uniqueID of decoding infos must be the same as parser groupId.
             */
            [ocrLineInfosArray addObject:[[PPDecodingInfo alloc] initWithLocation:ocrLineLocation
                                                                   dewarpedHeight:dewarpHeight
                                                                         uniqueId:US_CHEQUE_OCR_LINE]];
        }
        ...
        /** Setup image to be displayed */
        {
            ...
            [ocrLineInfosArray addObject:[[PPDecodingInfo alloc] initWithLocation:ocrLineLocation
                                                                   dewarpedHeight:fullDocumentDewarpHeight
                                                                         uniqueId:FULL_DOCUMENT_IMAGE]];
        }
        ...
        /**
         * Add decoding infos for classified results. These infos and their parsers will only be processed if classifier outputs the
         * selected class
         */
...
- (IBAction)didTapScan:(id)sender {
    ...
    UIViewController<PPScanningViewController> *scanningViewController =
        [PPViewControllerFactory cameraViewControllerWithDelegate:self
                                            overlayViewController:self.overlay
...
/**
 * Since we setup to return full dewarped image in coordinatorWithError:, this method will be called each time document is detected. Then,
 * we will store last found image to a variable so we can present it if needed
 */
    ...
    for (PPOcrBlock *block in [ocrResult blocks]) {
        for (PPOcrLine *line in [block lines]) {
            for (PPOcrChar *character in [line chars]) {
                ...
                if ([isSpaceChar characterIsMember:[character value]] || [character value] == '0' || [character value] == 'O' ||
                    [character value] == 'o') {
                    continue;
                }
...
- (void)showResultViewWithResult:(PPBlinkOcrRecognizerResult *)result {
    ...
    PPResultViewController *resultView =
        [[PPResultViewController alloc] initWithTitle:@"US CHEQUE" labels:dict subTitle:@"" labelOrder:nil];
    ...
    [self.scanningViewController.view addConstraint:[NSLayoutConstraint constraintWithItem:resultView.view
                                                                                 attribute:NSLayoutAttributeCenterX
                                                                                 relatedBy:NSLayoutRelationEqual
                                                                                    toItem:self.scanningViewController.view
                                                                                 attribute:NSLayoutAttributeCenterX
                                                                                multiplier:1.0
                                                                                  constant:0.0]];
    [self.scanningViewController.view addConstraint:[NSLayoutConstraint constraintWithItem:resultView.view
...
- (void)showResultImage {
    ...
    [self.scanningViewController.view addConstraint:[NSLayoutConstraint constraintWithItem:self.blurView
                                                                                 attribute:NSLayoutAttributeCenterX
                                                                                 relatedBy:NSLayoutRelationEqual
                                                                                    toItem:self.scanningViewController.view
                                                                                 attribute:NSLayoutAttributeCenterX
    ...
    [self.blurView addConstraint:[NSLayoutConstraint constraintWithItem:blurBackground
                                                              attribute:NSLayoutAttributeCenterX
                                                              relatedBy:NSLayoutRelationEqual
    ...
    [self.blurView addConstraint:[NSLayoutConstraint constraintWithItem:buttonClose
                                                              attribute:NSLayoutAttributeLeading
                                                              relatedBy:NSLayoutRelationEqual
                                                                 toItem:self.blurView
                                                              attribute:NSLayoutAttributeLeading
    ...
    [self.blurView addConstraint:[NSLayoutConstraint constraintWithItem:buttonSave
    ...
    [buttonClose addTarget:self action:@selector(closeResultImage) forControlEvents:UIControlEventTouchUpInside];
    [buttonSave addTarget:self action:@selector(blurViewSave) forControlEvents:UIControlEventTouchUpInside];
    ...
    [self.scanningViewController.view addConstraint:[NSLayoutConstraint constraintWithItem:self.succesfulImage
                                                                                 attribute:NSLayoutAttributeCenterX
                                                                                 relatedBy:NSLayoutRelationEqual
                                                                                    toItem:self.scanningViewController.view
                                                                                 attribute:NSLayoutAttributeCenterX
    ...
    NSLayoutConstraint *width = [NSLayoutConstraint constraintWithItem:self.succesfulImage
                                                             attribute:NSLayoutAttributeWidth
                                                             relatedBy:NSLayoutRelationEqual
                                                                toItem:self.scanningViewController.view
                                                             attribute:NSLayoutAttributeWidth
                                                            multiplier:0.0
                                                              constant:1.0];
    NSLayoutConstraint *height = [NSLayoutConstraint constraintWithItem:self.succesfulImage
                                                              attribute:NSLayoutAttributeHeight
                                                              relatedBy:NSLayoutRelationEqual
                                                                 toItem:self.scanningViewController.view
                                                              attribute:NSLayoutAttributeHeight
    ...
    width = [NSLayoutConstraint constraintWithItem:self.succesfulImage
                                         attribute:NSLayoutAttributeWidth
                                         relatedBy:NSLayoutRelationEqual
                                            toItem:self.scanningViewController.view
                                         attribute:NSLayoutAttributeWidth
                                        multiplier:0.9
                                          constant:0.0];
    width.priority = 999;
    height = [NSLayoutConstraint constraintWithItem:self.succesfulImage
                                          attribute:NSLayoutAttributeHeight
                                          relatedBy:NSLayoutRelationEqual
                                             toItem:self.scanningViewController.view
                                          attribute:NSLayoutAttributeWidth
...
- (void)blurViewSave {
    if (self.succesfulImage != nil) {
        UIImageWriteToSavedPhotosAlbum(self.imageMetadata.image, nil, nil, nil);
    }
}
