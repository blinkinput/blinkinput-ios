...
- (void)viewDidLoad {
    ...
    [buttonImage setTranslatesAutoresizingMaskIntoConstraints:NO];
    [buttonImage setTitle:@"Image" forState:UIControlStateNormal];
    [buttonImage setTitleColor:buttonTextColor forState:UIControlStateNormal];
    [buttonImage setTitleColor:[UIColor darkGrayColor] forState:UIControlStateHighlighted];
    buttonImage.titleLabel.font = buttonFont;
    buttonImage.contentEdgeInsets = buttonInsets;
    [self.view addSubview:buttonImage];

    [self.view addConstraint:[NSLayoutConstraint constraintWithItem:buttonImage
                                                          attribute:NSLayoutAttributeCenterY
                                                          relatedBy:NSLayoutRelationEqual
                                                             toItem:bottomView
                                                          attribute:NSLayoutAttributeCenterY
    ...
    [self.view addConstraint:[NSLayoutConstraint constraintWithItem:buttonImage
                                                         ...
                                                         multiplier:1.0
                                                           constant:0.0]];
    [buttonImage addTarget:self action:@selector(buttonImageDidTap:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)buttonImageDidTap:(UIButton *)sender {
    [self.delegate buttonMiddleDidTap:self];
}
